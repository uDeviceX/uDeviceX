...
the quantity. Let stem:[j \in [1, C_i]] denote the particle ids inside
cell stem:[i], where stem:[C_i] is the number of particles inside this
...
[stem]
++++
\mathbf{u}^i_t = \frac{1}{C_i} \sum\limits_{j = 1}^{C_i} \mathbf{u}_j
++++
...
[stem]
++++
S^i_{\alpha, \beta} = \frac{1}{V_i} \sum\limits_{j = 1}^{C_i} S_j^{\alpha, \beta}
++++
...
[stem]
++++
c^i_{\alpha} = \frac{1}{C_i} \sum\limits_{j = 1}^{C_i} \delta_{c_j, \alpha}
++++
