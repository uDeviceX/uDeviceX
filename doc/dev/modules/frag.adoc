...
The fragment stem:[(0, 0, 0)] is called _bulk_.
...
The major part of the functions are avalaible to both device and host,
...
----
Given subdomain sizes `L`, returns number of neighbouring cells to
fragment with id `i`
...
----
given a maximum number density `maxp`, return an estimate of the
