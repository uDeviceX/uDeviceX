...
void rig_update(float dt0, int n, const Force *ff, const float *rr0, int ns, /**/ Particle *pp, Solid *ss) {
    if (ns < 1) return;
    ...
    KL(dev::update_om_v, (1, ns), (dt0, ns, /**/ ss));
    if (!pin_com) KL(dev::update_com, (1, 3*ns ), (dt0, ns, /**/ ss));
    KL(dev::rot_referential, (1, ns), (dt0, ns, /**/ ss));
...
void rig_update_mesh(float dt0, int ns, const Solid *ss, int nv, const float *vv, /**/ Particle *pp) {
    const dim3 nblck ( ceiln(nv, NTHRD), ns );
    const dim3 nthrd ( NTHRD, 1 );
    KL(dev::update_mesh, (nblck, nthrd ), (dt0, ss, nv, vv, /**/ pp));
}
