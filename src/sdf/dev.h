...
static __device__ float3 grad(Sdf_v *texsdf, const float3 *pos) {
    int tc[3];
...
static __device__ float cheap_sdf(Sdf_v *texsdf, float x, float y, float z)  {
    int tc[3];
    float r[3] = {x, y, z};
    convert_round(r, /**/ tc);
...
static __device__ float sdf(Sdf_v *texsdf, float x, float y, float z) {
    int c;
