...
static __device__ void bulk2(float2 *pp, int i, int zplane, int n, float seed, /**/ float *ff, float *ff1) {
    Pa p;
    Fo f; /* "local" particle */
    Map m;
    p = pp2p(pp, i);
...
__global__ void bulk(float2 *pp, int n0, int n1, float seed, float *ff, float *ff1) {
    int gid, i, zplane;
    gid    = threadIdx.x + blockDim.x * blockIdx.x;
    i      = gid / 3;
    zplane = gid % 3;
    if (i >= n0) return;
    bulk2(pp, i, zplane, n1, seed, ff, ff1);
}
}
