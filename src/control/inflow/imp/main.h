...
static void create_solvent(float dt, Inflow *i, int *n, SolventWrap wrap) {
    int2 nc;
    Desc *d;
    int nctot;
...
void inflow_create_pp(float dt, Inflow *i, int *n, Particle *pp) {
    SolventWrap wrap;
    ...
    create_solvent(dt, i, n, wrap);
}

void inflow_create_pp_cc(float dt, int newcolor, Inflow *i, int *n, Particle *pp, int *cc) {
    SolventWrap wrap;
