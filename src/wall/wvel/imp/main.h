...
static void set_dev(WvelHS p, Wvel_v *wv) {
    wv->type = WALL_VEL_V_HS;    
    wv->p.hs.u = p.u;
    wv->p.hs.h = p.h;
}

void wvel_get_view(float dt0, long it, const Wvel *wv, /**/ Wvel_v *view) {
    switch (wv->type) {
    case WALL_VEL_CSTE:
    ...
    case WALL_VEL_SHEAR_SIN:
        set_dev(dt0, it, wv->p.shearsin, /**/ view);
        break;
    case WALL_VEL_HS:
        set_dev(wv->p.hs, /**/ view);
        break;
    default:
