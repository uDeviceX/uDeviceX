...
void write_all(MPI_Comm cart, const void * const ptr, const int nbytes32, WriteFile *fp) {
    MPI_File f = fp->f;
    MPI_Offset base;
...
int write_master(MPI_Comm cart, const void * const ptr, int sz0, WriteFile *fp) {
    int sz;
    sz = m::is_master(cart) ? sz0 : 0;
    write_all(cart, ptr, sz, fp);
    return 0;
}

int write_shift_indices(MPI_Comm cart, int n, int *shift0) {
    *shift0 = 0;
