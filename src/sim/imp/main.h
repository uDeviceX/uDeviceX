...
void sim_gen(Sim *s, Config *cfg, Time *time, float tend) {
    float dt;
    Flu *flu = &s->flu;
    Rbc *rbc = &s->rbc;
    Wall *wall = &s->wall;
    ...
    if (walls || solids) {
        ...
        run(cfg, time, wall_creation*dt, tend, s);
    } else {
        s->solids0 = solids;
        run(cfg, time, 0, tend, s);
    }
    /* final strt dump*/
    if (strt_dumps) dump_strt(RESTART_FINAL, s);
}

void sim_strt(Sim *s, Config *cfg, Time *time, float tend) {
    float dt;
    Flu *flu = &s->flu;
    Rbc *rbc = &s->rbc;
    Rig *rig = &s->rig;
