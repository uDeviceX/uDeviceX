...
void sim_gen(Sim *s) {
    ...
    flu::flu_gen_quants(s->coords, &flu->q);
    flu::flu_build_cells(&flu->q);
    if (global_ids)    flu::flu_gen_ids  (s->cart, flu->q.n, &flu->q);
    if (rbcs) {
...
void sim_strt(Sim *s) {
    ...
    /*Q*/
    flu::flu_strt_quants(s->coords, restart::BEGIN, &flu->q);
    flu::flu_build_cells(&flu->q);
