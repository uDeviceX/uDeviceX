...
void sim_gen(Sim *s) {
    ...
    /* final strt dump*/
    if (strt_dumps) dump_strt(restart::RESTART_FINAL, s);
}
...
void sim_strt(Sim *s) {
    ...
    /*Q*/
    flu_strt_quants(s->coords, restart::RESTART_BEGIN, &flu->q);
    flu_build_cells(&flu->q);

    if (rbcs) rbc_strt_quants(s->coords, "rbc.off", restart::RESTART_BEGIN, &rbc->q);
    dSync();

    if (solids) rig_strt_quants(s->coords, restart::RESTART_BEGIN, &rig->q);
