...
void forces_wall(Wall *w, Sim *s) {
    ...
    if (flu->q.n)               color::wall_force(w->vview, s->coords, w->sdf, &w->q, w->t, co, flu->q.n, /**/ flu->ff);
    if (s->solids0 && rig->q.n) grey::wall_force(w->vview, s->coords, w->sdf, &w->q, w->t, cs, rig->q.n, /**/ rig->ff);
    if (rbcs && rbc->q.n)       grey::wall_force(w->vview, s->coords, w->sdf, &w->q, w->t, cr, rbc->q.n, /**/ rbc->ff);
}
