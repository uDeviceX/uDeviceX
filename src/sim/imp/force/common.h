...
void forces_wall(Wall *w) {
    ...
    if (flu.q.n)           color::force(w->sdf, w->q, w->t, co, flu.q.n, /**/ flu.ff);
    if (solids0 && rig.q.n) grey::force(w->sdf, w->q, w->t, cs, rig.q.n, /**/ rig.ff);
    if (rbcs && rbc.q.n)    grey::force(w->sdf, w->q, w->t, cr, rbc.q.n, /**/ rbc.ff);
}
