...
static void dump_part(int step, Sim *s) {
    ...
    if (global_ids) {
        ...
        bop::bop_ids(s->cart, flu->q.ii_hst, flu->q.n, "id_solvent", step);
    }
    if (multi_solvent) {
        cD2H(flu->q.cc_hst, flu->q.cc, flu->q.n);
        bop::bop_colors(s->cart, flu->q.cc_hst, flu->q.n, "colors_solvent", step);
    }

    if (force_dumps) {
        cD2H(flu->ff_hst, flu->ff, flu->q.n);
        bop::bop_parts_forces(s->cart, s->coords, flu->q.pp_hst, flu->ff_hst, flu->q.n, "solvent", step, /**/ dumpt);
    } else {
        bop::bop_parts(s->cart, s->coords, flu->q.pp_hst, flu->q.n, "solvent", step, /**/ dumpt);
    }
    ...
    if(s->solids0) {
        ...
        if (force_dumps) {
            ...
            bop::bop_parts_forces(s->cart, s->coords, rig->q.pp_hst, rig->ff_hst, rig->q.n, "solid", step, /**/ dumpt);
        } else {
            bop::bop_parts(s->cart, s->coords, rig->q.pp_hst, rig->q.n, "solid", step, /**/ dumpt);
        }
    }
}
