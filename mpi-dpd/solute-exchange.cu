...
__global__ void scatter_indices(const float2 *const particles,
                                const int nparticles, int *const counts) {
...
__global__ void tiny_scan(const int *const counts,
                          const int *const oldtotalcounts,
                          int *const totalcounts, int *const paddedstarts) {
...
__global__ void pack(const float2 *const particles, const int nparticles,
                     float2 *const buffer, const int nbuffer,
                     const int soluteid) {
...
#define _ACCESS(x) (*(x))
  ...
  for (int localbase = 32 * (warpid + 4 * blockIdx.x); localbase < npack_padded;
    ...
    const int key1 = (int)(localbase >= cpaddedstarts[key9 + key3 + 1]) +
...
__global__ void unpack(float *const accelerations, const int nparticles) {
  ...
  for (int gid = threadIdx.x + blockDim.x * blockIdx.x; gid < 3 * npack_padded;
    ...
    const int key1 = (int)(pid >= cpaddedstarts[key9 + key3 + 1]) +
