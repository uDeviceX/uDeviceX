...
void Simulation::_forces(bool firsttime)
{
    ...
    { // TSS
        ...
        for (int sstep = 0; sstep < nsubsteps; sstep++)
        {
            ...
            if (rbcscoll)
            {
                if (sstep > 0) CUDA_CHECK( cudaMemcpy(rbcscoll->acc(), rbcscoll->fsiacc(), 3*rbcscoll->pcount()*sizeof(float), cudaMemcpyDeviceToDevice) );
                rbc_interactions.internal_forces(rbcscoll->data(), rbcscoll->count(), rbcscoll->acc(), mainstream);
            ...
            if (ctcscoll)
            {
                if (sstep > 0) CUDA_CHECK( cudaMemcpy(ctcscoll->acc(), ctcscoll->fsiacc(), 3*ctcscoll->pcount()*sizeof(float), cudaMemcpyDeviceToDevice) );
                ctc_interactions.internal_forces(ctcscoll->data(), ctcscoll->count(), ctcscoll->acc(), mainstream);
