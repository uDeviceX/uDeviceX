...
static void sim_redistribute() {
  ...
  if (rbcs) RedistRBC::extent(rbcscoll->pp->D, Cont::ncells);
  RedistPart::send();
  if (rbcs) RedistRBC::pack_sendcount(rbcscoll->pp->D, Cont::ncells);
  ...
  if (rbcs) RedistRBC::unpack(rbcscoll->pp->D, Cont::ncells);
}
