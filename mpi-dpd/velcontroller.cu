    ...
    __global__ void sample(const int * const __restrict__ cellsstart, const float2* const __restrict__ p, float3* res, CellInfo info)
    {
        ...
        if (ccoos.x < info.n[0] && ccoos.y < info.n[1] && ccoos.z < info.n[2])
        {
            ...
            for (uint pid = cellsstart[cid]; pid < cellsstart[cid+1]; pid++)
            {
                ...
                res[resid].x += tmp1.y / num;
                res[resid].y += tmp2.x / num;
                res[resid].z += tmp2.y / num;
            }
        }
    }
...
void VelController::sample(const int * const cellsstart, const Particle* const p, cudaStream_t stream)
{
    ...
    sampleid++;
    if (total)
        VelControlKernels::sample <<<grid, block, 0, stream>>> (cellsstart, (float2*)p, vel.data, info);
}
...
float3 VelController::adjustF(cudaStream_t stream)
{
    ...
    if (total)
        CUDA_CHECK( cudaMemset(vel.data, 0, total * sizeof(float3)) );
    sampleid = 0;
