namespace SolEx {
  ...
  void init(MPI_Comm _cartcomm) {
    ...
    for (int i = 0; i < 26; ++i) {
      ...
      CC(cudaMemcpyToSymbol(k::rex::ccapacities,
			    &local[i]->scattered_indices->C, sizeof(int),
			    sizeof(int) * i, cudaMemcpyHostToDevice));
      CC(cudaMemcpyToSymbol(k::rex::scattered_indices,
  ...
  void _pack_attempt() {
    ...
    for (int i = 0; i < wsolutes.size(); ++i) {
      ...
      if (it.n) {
	CC(cudaMemcpyToSymbolAsync(k::rex::coffsets, packsoffset->D + 26 * i,
				   sizeof(int) * 26, 0, cudaMemcpyDeviceToDevice));

	k::rex::scatter_indices<<<(it.n + 127) / 128, 128, 0>>>(
									   (float2 *)it.p, it.n, packscount->D + i * 26);
      }

      k::rex::tiny_scan<<<1, 32, 0>>>(
    ...
    CC(cudaMemcpyToSymbolAsync(k::rex::cbases, packstotalstart->D,
	...
	CC(cudaMemcpyToSymbolAsync(k::rex::coffsets, packsoffset->D + 26 * i,
				   sizeof(int) * 26, 0, cudaMemcpyDeviceToDevice));
	CC(cudaMemcpyToSymbolAsync(k::rex::ccounts, packscount->D + 26 * i,
				   sizeof(int) * 26, 0, cudaMemcpyDeviceToDevice));
	CC(cudaMemcpyToSymbolAsync(k::rex::cpaddedstarts,
				   packsstart->D + 27 * i, sizeof(int) * 27, 0,
				   cudaMemcpyDeviceToDevice));

	k::rex::pack<<<14 * 16, 128, 0>>>
  ...
  void post_p() {
	...
	CC(cudaMemcpyToSymbolAsync(k::rex::ccapacities, newcapacities,
	...
	CC(cudaMemcpyToSymbolAsync(k::rex::scattered_indices, newindices,
  ...
  void recv_a() {
...

    {
      ...
      CC(cudaMemcpyToSymbolAsync(k::rex::recvbags, recvbags, sizeof(recvbags),
	...
	k::rex::unpack<<<16 * 14, 128, 0>>>((float *)it.a, it.n);
      }
