...
namespace CudaRBC
{
  ...
  std::vector<int> extract_neighbors(std::vector<int> adjVert, const int degreemax, const int v)
    {
	...
	std::vector<int> adjVert2(degreemax * nvertices, -1);
	  ...
	  std::vector<int> myneighbors = extract_neighbors(adjVert, degreemax, v);
            ...
            {
	      std::vector<int> s1 = extract_neighbors(adjVert, degreemax, myneighbors[i]);
	      std::sort(s1.begin(), s1.end());

	      std::vector<int> s2 = extract_neighbors(adjVert, degreemax, myneighbors[(i + 1) % myneighbors.size()]);
