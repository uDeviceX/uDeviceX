...
void collageSDF(int NX, int NY, const vector< vector<float> >& sampleSDF, vector<float>& outputSDF)
{
...
void collageSDFWithWall(const int NX, const int NY, const float xextent, const float yextent, 
                ...
                const float paddingAdd, vector<float>& outputSDF)
{        
    collageSDF(NX, NY, sampleSDF, outputSDF);
    const int xtimes = 1;
    ...
    float widthOfBufferZone = paddingAdd - wallWidth; 
